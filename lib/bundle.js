...
var renderYear = exports.renderYear = function renderYear() {
  d3.csv('./lib/year.csv', function (d) {
    ...
    bar.selectAll('text').data(data).enter().append('text').attr('x', function (d) {
      ...
      return y(d.Total) + 20;
    }).attr('fill', 'white').transition().delay(2000).text(function (d) {
      return '' + d.Total;
    });
  });
};
...
var renderMap = exports.renderMap = function renderMap(error, data, breeder, dog) {
  ...
  var drawPath = function drawPath(org, des) {
    g.append('path').datum({ type: 'LineString', coordinates: [[+org.lng, +org.lat], [+des.lng, +des.lat]] }).attr('class', 'route').attr('d', function (d) {
      return path(d);
    }).transition().duration(2000).style('fill', 'none').style('stroke', colorScale(org.id)).style('stroke-width', 2);
  };
};
