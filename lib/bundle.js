...
$(document).ready(function () {
  ...
  $(window).on('resize', function () {
    return (0, _bar.renderYear)(i++);
  });
  $(window).on('scroll', function () {
    return window.pageYOffset > 5 ? $('.top').fadeIn(800) : $('.top').fadeOut(800);
  });
});

$('.buttons #year').click(function () {
  return $('body, html').animate({ scrollTop: $('.year').offset().top - 20 }, "slow");
});
$('.buttons #map').click(function () {
  return $('body, html').animate({ scrollTop: $('.map').offset().top - 20 }, "slow");
});
$('.buttons #sunburst').click(function () {
  return $('body, html').animate({ scrollTop: $('.sunburst').offset().top - 20 }, "slow");
});
$('.top').click(function () {
  return $('body, html').animate({ scrollTop: 0 }, "slow");
});
...
var i = 1;
$(window).on('resize', function () {
  return renderSunburst(i++);
});

var renderSVG = function renderSVG() {
  var _getDemension = getDemension(),
      outerWidth = _getDemension.outerWidth,
      outerHeight = _getDemension.outerHeight,
      margin = _getDemension.margin,
      radiu = _getDemension.radiu;

  svg = svg || d3.select('.sunburst').append('svg').attr('width', outerWidth).attr('height', outerHeight);

  imgG = imgG || svg.append('clipPath').attr('id', 'imgG').attr('transform', 'translate(120, 120)');

  return { svg: svg, imgG: imgG };
};

var getDemension = function getDemension() {
  var outerWidth = $('.sunburst').width();
  var outerHeight = $('.sunburst').height();
  var margin = { top: 10, right: 10, bottom: 10, left: 10 };
  var radius = Math.min(outerWidth, outerHeight) * 0.35;
  return { outerWidth: outerWidth, outerHeight: outerHeight, margin: margin, radius: radius };
};

var renderSunburst = exports.renderSunburst = function renderSunburst() {
  var _renderSVG = renderSVG(),
      svg = _renderSVG.svg,
      imgG = _renderSVG.imgG;

  var _getDemension2 = getDemension(),
      outerWidth = _getDemension2.outerWidth,
      outerHeight = _getDemension2.outerHeight,
      margin = _getDemension2.margin,
      radius = _getDemension2.radius;
...
var margin = { top: 150, right: 20, bottom: 10, left: 20 };

var svg = d3.select('.map').append('svg').attr('id', 'map-svg').attr('width', width - margin.right - margin.left).attr('height', height - margin.top);

var g = d3.select('#map-svg').append('g').attr('transform', 'translate(' + margin.left + ', ' + margin.top + ')');
...
var renderMap = exports.renderMap = function renderMap(error, data, breeder, dog) {
  ...
  var nodeEnter = node.enter().append('svg:g').attr('class', 'breeder').attr('id', function (d) {
    ...
    return clearDogAndRoute();
  });

  var clearDogAndRoute = function clearDogAndRoute() {
  ...
  var drawPathNew = function drawPathNew(org, des) {
    ...
    g.append('path').attr('class', 'route').attr('d', lineGenerator(point)).style('fill', 'none').style('stroke', colorScale(org.id)).style('stroke-width', 1).call(transition);
  };

  var drawPath = function drawPath(org, des) {
    g.append('path')
    // getMidPoint(org, des),
    .datum({ type: 'LineString', coordinates: [[+org.lng, +org.lat], [+des.lng, +des.lat]] }).attr('class', 'route').attr('d', linePath).style('fill', 'none').style('stroke', colorScale(org.id)).style('stroke-width', 1).call(transition);
  };
...
var updateDeminsion = function updateDeminsion() {
  var outerWidth = $('.year').width();
  var outerHeight = $('.year').height();
  var margin = { top: 10, right: 10, bottom: 100, left: 40 };
  var innerWidth = outerWidth - margin.left - margin.right;
  var innerHeight = outerHeight - margin.top - margin.bottom;
  return { outerWidth: outerWidth, outerHeight: outerHeight, margin: margin, innerWidth: innerWidth, innerHeight: innerHeight };
};

var renderSVG = function renderSVG() {
  var _updateDeminsion = updateDeminsion(),
      outerWidth = _updateDeminsion.outerWidth,
      outerHeight = _updateDeminsion.outerHeight,
      margin = _updateDeminsion.margin,
      innerWidth = _updateDeminsion.innerWidth,
      innerHeight = _updateDeminsion.innerHeight;

  svg = svg || d3.select('.year').append('svg').attr('id', 'year-chart').attr('width', outerWidth).attr('height', outerHeight);
  return { svg: svg };
};

var renderYear = exports.renderYear = function renderYear(idx) {
  var _updateDeminsion2 = updateDeminsion(),
      outerWidth = _updateDeminsion2.outerWidth,
      outerHeight = _updateDeminsion2.outerHeight,
      margin = _updateDeminsion2.margin,
      innerWidth = _updateDeminsion2.innerWidth,
      innerHeight = _updateDeminsion2.innerHeight;
  ...
  g = g || svg.append('g').attr('width', innerWidth).attr('height', innerHeight).attr('transform', 'translate(' + margin.left + ', ' + margin.top + ')');
  ...
  d3.csv('./lib/year.csv', function (d) {
    ...
    var y = d3.scaleLinear().domain(d3.min(data), d3.max(data)).rangeRound([0, innerHeight]);
    ...
    // .call(d3.axisLeft(y))
    .append('text').attr('transform', 'rotate(-90)').attr('y', 6).attr('dy', '0.71em').attr('text-anchor', 'end').text('Total (Million)');

    var barEnter = g.append('g').attr('class', 'bars').selectAll('.bar').data(data);
    barEnter.enter().append('rect').attr('class', 'bar').attr('id', function (d) {
      return 'year' + d.Year;
    }).attr('x', function (d) {
      return x(d.Year);
    }).attr('width', x.bandwidth()).attr('y', i ? function (d) {
      return innerHeight - y(d.Total);
    } : innerHeight).attr('height', i ? function (d) {
      return y(d.Total);
    } : 0).call(i ? dummy : transition);
    ...
    function transition(bar) {
      bar.transition().duration(200).delay(function (d, i) {
        return i * 200;
      }).attr('y', function (d) {
        return innerHeight - y(d.Total);
    ...
    g.selectAll('.bar').on('click', function (d) {
      var div = document.querySelector('.monthly-pie-append');
      div.classList.add('extend');
      $('body, html').animate({ scrollTop: $(div).offset().top - 20 }, "slow");
      (0, _line.appendMonthly)(d);
      (0, _pie.appendPie)(d);
    });

    barEnter.enter().append('text').attr('class', 'bar-text').attr('x', function (d) {
      return x(d.Year) + x.bandwidth() / 4;
    }).attr('y', function (d) {
      return innerHeight - y(d.Total) + 20;
    }).attr('fill', 'white').transition().duration(2000).delay(i ? 0 : 2000).text(function (d) {
      return '' + d.Total;
    });
  });
};
