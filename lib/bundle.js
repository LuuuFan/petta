...
queue().defer(d3.json, '../data/us-full.json').defer(d3.csv, '../data/breeder.csv').defer(d3.csv, '../data/breeder-dog.csv').await(_map.renderMap);
...
var renderMonthly = function renderMonthly(data) {
  ...
  path.attr('d', line(lineData)).transition().duration(1000).style('fill', 'none').style('stroke', 'black').style('stroke-width', '1px');
};
...
var renderSunburst = exports.renderSunburst = function renderSunburst() {
  ...
  var radius = Math.min(outerWidth, outerHeight) * 0.4;
  var colorScale = d3.scaleOrdinal(d3.schemeCategory10);
  var totalSize = 0;
  ...
  svg.append('svg:image').attr('id', 'img').attr('clip-path', 'url(#imgG)').attr('height', 250).attr('width', 250).style('opacity', 0.8);
  ...
  d3.json('../data/sunburst-2.json', function (error, data) {
    if (error) throw error;
  ...
  var mouseOver = function mouseOver(d) {
    ...
    if (d.data.img) {
      d3.select('#img').attr("xlink:href", '' + d.data.img);
    } else {
      d3.select('#img').attr("xlink:href", "");
    }
...
var renderMap = exports.renderMap = function renderMap(error, data, breeder, dog) {
  ...
  g.selectAll('.breeder').data(breeder).enter().append('circle').attr('class', 'breeder').attr('id', function (d) {
    return 'breeder-' + d.id;
  }).attr('r', 8).attr('cx', function (d) {
    var geocode = projection([d.lng, d.lat]);
    return geocode[0];
  }).attr('cy', function (d) {
    var geocode = projection([d.lng, d.lat]);
    return geocode[1];
    ...
    return colorScale(d.id);
  }).attr('fill-opacity', 0.7).on('mouseover', function (d) {
    return drawDog(d);
  }).on('mouseleave', function () {
  ...
  var drawPath = function drawPath(org, des) {
    g.append('path').datum({ type: 'LineString', coordinates: [[+org.lng, +org.lat], [+des.lng, +des.lat]] }).transition().duration(2000).attr('class', 'route').attr('d', function (d) {
      return path(d);
    }).style('fill', 'none').style('stroke', colorScale(org.id)).style('stroke-width', 2);
  };
};
