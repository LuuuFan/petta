...
var renderMap = exports.renderMap = function renderMap(error, data, breeder, dog) {
  ...
  var drawPath = function drawPath(org, des) {
    g.append('path').datum({ type: 'LineString', coordinates: [[+org.lng, +org.lat], getMidPoint(org, des), [+des.lng, +des.lat]] }).attr('class', 'route').attr('d', linePath).style('fill', 'none').style('stroke', colorScale(org.id)).style('stroke-width', 2).call(transition);
  };

  g.selectAll('.route').attr('d', function (d) {
...
var transition = function transition(line) {
